...
SELECT DISTINCT ON(lastname) * FROM employees; --4
...
SELECT department FROM employees GROUP BY department HAVING count(*) > 2; --11

SELECT name FROM departments ORDER BY budget DESC LIMIT 1 OFFSET 1;  --12

SELECT name, lastname FROM employees WHERE department = (SELECT code FROM departments ORDER BY budget ASC LIMIT 1);  --13

SELECT name FROM employees WHERE city = 'Almaty' UNION ALL SELECT name FROM customers WHERE city = 'Almaty'; --14

SELECT name FROM departments WHERE budget > 60000 ORDER BY budget ASC, code DESC ;  --15

UPDATE departments SET budget = budget - budget * 0.1 WHERE budget = (SELECT budget FROM departments ORDER BY budget ASC LIMIT 1) RETURNING *; --16

UPDATE employees SET department = 14 WHERE department = 77 RETURNING *;  --17

DELETE FROM employees AS e WHERE e.department = 14; --18
